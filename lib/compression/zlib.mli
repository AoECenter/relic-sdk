...
type algo = Deflated (** Compression algorithm. Only deflate is provided by current zlib. *)
...
type 'a t =
  { state : 'a state
  ; mutable in_buf : bigstring (** bigstring input buffer *)
  ; mutable out_buf : bigstring (** bigstring output buffer *)
  ; mutable in_ofs : int (** offset into the input buffer *)
  ; mutable out_ofs : int (** offset into the output buffer *)
  ; mutable in_len : int (** Length of available input data *)
  ; mutable out_len : int (** Length of available output data *)
...
(** [create_inflate ()]
    ...
    decompression.
    @param window_bits
      the base 2 logarithm of the maximum window size. It
...
(** [create_deflate ()] Creates zlib internal {!state} and buffer description
    for compression.
    @param level
      the compression level must be betweed [-1] and [9].
    ...
    @param memory
      selects how much memory to use for compression in the
...
(** [flate buffers flush] (de)compresses data from the provided input to the
    output buffers.
    @return
      {!status.Ok} if some progress has been made but more input or
    ...
    @param flush
      Whether and how the output should be flushed.
...
(** [inflate_set_dictionary state dict] Sets a preset dictionary for
    ...
    required dictionary.
    @return
      {!status.Data_error} when the adler32 checksum of the provided
