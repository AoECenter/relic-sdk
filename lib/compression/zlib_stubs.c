...
value zlib_adler32(value vadler, value vbuf) {
  return caml_copy_int32(adler32(Int32_val(vadler), (Bytef *)String_val(vbuf),
...
CAMLprim value zlib_deflate_init(value level, value method, value windowBits,
                                 value memLevel, value strategy) {
  ...
  */
  ...
  vwrap = caml_alloc_final(WRAP_STRM_WOSIZE, zlib_finalize, memory,
                           ZLIB_MAX_MEMORY);
  ...
  zlib_error(strm,
             deflateInit2(strm, Int_val(level), Z_DEFLATED, Int_val(windowBits),
                          Int_val(memLevel), Int_val(strategy)));
...
CAMLprim value zlib_deflate_set_dictionary(value vstrm, value vdict) {
  ...
  zlib_error(strm, deflateSetDictionary(strm, (Bytef *)String_val(vdict),
...
CAMLprim value zlib_inflate_set_dictionary(value vstrm, value vdict) {
  ...
  CAMLreturn(
      zlib_error(strm, inflateSetDictionary(strm, (Bytef *)String_val(vdict),
                                            caml_string_length(vdict))));
}
...
CAMLprim value zlib_set_header(value vstrm, value vheader) {
  ...
  zlib_error(strm, deflateSetHeader(strm, header));
...
CAMLprim value zlib_get_header(value vstrm) {
  ...
  if (header->extra != NULL) {
    len = header->extra_len < header->extra_max ? header->extra_len
                                                : header->extra_max;
...
#define LField(n) Long_val(Field(vstrm, (n)))
  ...
  if (LField(3) < 0 || LField(4) < 0 || LField(5) > strm->avail_in ||
      LField(6) > strm->avail_out)
